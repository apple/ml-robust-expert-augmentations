    ...
    for epoch in range(config["n_epochs"]):
        for i in range(n_iter):
            ...
            for x_train in tqdm(dl_train):
                ...
                loss = loss / baseline
                if trained_model is not None and config.get("loss_params", True):
            ...
            with torch.no_grad():
                ...
                if solver == "APHYNITYDoublePendulum":
                ...
                if best_valid > cur_valid:
                    ...
                    elif solver == "APHYNITYDoublePendulum":
...
path_models = config["optimization"]["path_model"][:-3]
for i in range(nb_exp):
    print("------------- Robustness of Augmentation experiment %d -------------" % i)
    try:
        config["optimization"]["path_model"] = path_models + str(i) + ".pt"
        save_pth = pth + "/robustness_%d_" % i
        run_exp(s, True, config=config["optimization"], solver=config["optimization"].get("model", "None"), config_name=config_name,
                data_path=data_path, save_path=save_pth)
    except Exception as e:
        print(e)
        print("skipping identifier %d" % i)
